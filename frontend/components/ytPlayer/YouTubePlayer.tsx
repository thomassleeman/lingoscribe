...
// Singleton pattern for YouTube API loading
// This ensures multiple player instances don't conflict with the global callback
function loadYouTubeApi(callback: () => void) {
  ...
  // If this is the first caller, load the API
  if (window.__ytApiCallbacks.length === 1) {
    ...
    // Set up the global callback to call ALL queued callbacks
...
export default function YouTubePlayer({ videoId }: YouTubePlayerProps) {
  ...
  const maxInitAttempts = 50; // ~5 seconds of retrying
  ...
  const initializePlayer = useCallback(() => {
    ...
    if (rect.height === 0 || rect.width === 0) {
      ...
      if (initAttempts.current >= maxInitAttempts) {
        console.warn(
          "YouTubePlayer: Container never got dimensions after",
          maxInitAttempts,
          "attempts"
        );
        return;
  ...
  // Re-initialize if videoId changes
  useEffect(() => {
  ...
  const onPlayerReady = (event: any) => {
    ...
    // Set up interval to track playback time
  ...
  return (
    <div className="h-full w-full bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 flex flex-col">
      {/* Video Container */}
      <div
        ref={containerRef}
        className="w-full flex-1 bg-black min-h-[200px]"
      />
...
//   return (
//     <div className="w-full bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 flex flex-col">
//       {/* Video Container */}
//       <div
//         ref={containerRef}
//         className="w-full flex-1 bg-black relative h-auto aspect-video"
//       />
