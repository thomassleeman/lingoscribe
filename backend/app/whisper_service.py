...
#     try:
#         # Read the file in async manner
#         async with aiofiles.open(file_path, 'rb') as audio_file:
#             audio_bytes = await audio_file.read()

#         # Since OpenAI client does not support async, use run_in_executor
#         def transcribe():
#             with open(file_path, "rb") as audio_file_sync:
#                 response = client.audio.transcriptions.create(
#                     file=audio_file_sync,
#                     model="whisper-1",
#                     response_format="verbose_json"
...
#         # Use the client_id to make the filename unique
#         unique_filename = f"yt_{client_id}"
#         output_path = os.path.join(UPLOAD_FOLDER, f"{unique_filename}.%(ext)s")
#         await download_youtube_audio(url, output_path, client_id)

#         # Find the downloaded file
#         downloaded_files = [f for f in os.listdir(UPLOAD_FOLDER) if f.startswith(unique_filename)]
#         if not downloaded_files:
#             raise Exception("Audio file not found after download.")

#         audio_file_path = os.path.join(UPLOAD_FOLDER, downloaded_files[0])
#         audio_url = f"/audio/{downloaded_files[0]}"

#         # Step 2: Transcribe the audio
#         transcript_data = await transcribe_audio(audio_file_path, client_id)

#         if client_id:
#             await send_progress("YouTube video processing complete.", client_id)
#         print("YouTube video processing complete.")

#         return transcript_data, audio_url, url
#     except Exception as e:
#         error_message = f"Error processing YouTube video: {str(e)}"
#         print(traceback.format_exc())
#         if client_id:
